...
Buy values go negative when price stretches below recent lows. Sell values go positive when price stretches above recent highs. The longest lookback that triggers wins, so a 132-day signal is not also counted as 66 or 22. An optional CBOE VIX series can be plotted on the same pane, offset for display, with a highlight when VIX is above its threshold.
...
| Enable Buy / Sell Signals | true / false | Sell is off by default |
| Enable VIX Signal | true | Plots VIX offset by +100 |
| VIX Signal Threshold | 30 | Highlight when raw VIX is above this level |
| Show Backtest Table | false | Win/loss counts after the trade duration |
